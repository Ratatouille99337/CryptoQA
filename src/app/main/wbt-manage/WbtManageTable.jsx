...
function WbtManageTable() {
	...
	const columns = useMemo(
		() => [
			{
				accessorKey: 'domain',
				header: 'First Name', // Assuming 'domain' is actually first name
				accessorFn: (row) => `${row.domain}`
			},
			{
				accessorKey: 'name',
				header: 'Last Name', // Corrected header
				accessorFn: (row) => `${row.name}`
			},
			{
				accessorKey: 'email',
				header: 'Email', // Trimmed extra space
	...
	const deleteSeletedUsers = (emails) => {
		...
		axios.post(`${devApiLink}/user/deletes`, { emails })
		.then((response) => {
	...
	const deleteSeletedUser = (email) => {
		axios.post(`${devApiLink}/user/delete`, { email })
		.then((response) => {
			console.log(response);
